...
//----------------------------------------------------------------------------
bool vtkCjyxDynamicModelerAppendTool::RunInternal(vtkDMMLDynamicModelerNode* surfaceEditorNode)
{
  ...
  vtkDMMLModelNode* outputModelNode = vtkDMMLModelNode::SafeDownCast(surfaceEditorNode->GetNodeReference(APPEND_OUTPUT_MODEL_REFERENCE_ROLE));
  if (!outputModelNode)
    {
    // Nothing to output
    return true;
  ...
  for (int i = 0; i < numberOfInputNodes; ++i)
    {
    vtkDMMLNode* inputNode = surfaceEditorNode->GetNthNodeReference(APPEND_INPUT_MODEL_REFERENCE_ROLE, i);
    vtkDMMLModelNode* modelNode = vtkDMMLModelNode::SafeDownCast(inputNode);
    if (!modelNode)
      {
      continue;
      }
